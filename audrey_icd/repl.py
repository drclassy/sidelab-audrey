"""REPL handler untuk command `/icd` di AUDREY.
...
def handle_icd_command(user_input: str, console) -> None:
    ...
    if not arg:
        ...
        console.print(f"  [bold]Kamus ICD-10 Indonesia[/bold] — {total} kode tersedia", style="grey82")
        console.print("  Penggunaan:", style="dim grey50")
        console.print("    [cyan]/icd I10[/cyan]            lookup berdasarkan kode", style="grey82")
        console.print("    [cyan]/icd E11[/cyan]            kode parent — tampil semua sub-kode", style="grey82")
        console.print("    [cyan]/icd hipertensi[/cyan]     cari berdasarkan nama (ID/EN)", style="grey82")
        console.print()
        return
    ...
    if is_code_query(arg):
        ...
        if direct:
            ...
            console.print(f"  [bold]{arg.upper()} family[/bold] [dim]({len(kids)} sub-kode)[/dim]", style="grey82")
            for kid in kids:
    ...
    if results:
        console.print(f"  [bold]Hasil pencarian:[/bold] {arg!r} [dim]({len(results)} hasil)[/dim]", style="grey82")
