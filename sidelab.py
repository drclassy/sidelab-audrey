...
# ---------------------------------------------------------------------------
# System prompt — SIDELAB Protocol v1
# ---------------------------------------------------------------------------
def _build_system(pasien: dict) -> str:
    ...
    return f"""{pasien_str}Kamu adalah SIDELAB, asisten klinis untuk dokter FKTP/Puskesmas Indonesia oleh Sentra SideLab Project.
...
Jika ditanya identitas: "Saya SIDELAB, Clinical Intelligence oleh Sentra SideLab Project."
...
def main() -> None:
    ...
    _spec = PROVIDER_REGISTRY.get(backend, {})
    _client_type = _spec.get("client")
    if _client_type == "local" and not local_available_models():
        console.print(
            "  [i] Ollama atau model lokal belum terdeteksi. Mode Local akan gagal sampai tersedia.",
            style="bright_yellow",
        )
    elif _client_type == "gemini_vertex":
        if not (os.getenv("VERTEX_PROJECT") or os.getenv("GOOGLE_CLOUD_PROJECT", "")).strip():
            console.print(
                "  [i] VERTEX_PROJECT belum diisi. Lengkapi .env agar Gemini Vertex bisa dipakai.",
                style="bright_yellow",
            )
    elif _client_type == "openai_compat":
        _api_key_env = _spec.get("api_key_env", "")
        if _api_key_env and not os.getenv(_api_key_env, "").strip():
            console.print(
                f"  [i] {_api_key_env} belum diisi. Lengkapi .env agar {_spec.get('label', backend)} bisa dipakai.",
                style="bright_yellow",
            )
    console.print(
