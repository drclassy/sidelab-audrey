...
class OpenAICompatClientTests(unittest.TestCase):
    ...
    def test_raises_on_empty_api_key(self, mock_post):
        client = OpenAICompatClient(name="kimi", label="Kimi", base_url="https://api.moonshot.cn/v1", api_key="")
        with self.assertRaises(RuntimeError):
            list(client.stream_chat([{"role": "user", "content": "hi"}], "moonshot-v1-8k"))
        mock_post.assert_not_called()

    @patch("sidelab.llm.openai_compat_client.requests.post")
    def test_raises_on_http_error(self, mock_post):
        mock_post.return_value = FakeResponse(status_code=401, text='{"error": "Unauthorized"}')
        client = OpenAICompatClient(name="qwen", label="Qwen", base_url="https://dashscope.aliyuncs.com/compatible-mode/v1", api_key="x")
