...
class InstallerScaffoldTests(unittest.TestCase):
    ...
    def test_inno_script_references_payload_and_shortcuts(self):
        script = (ROOT / "installer" / "audrey.iss").read_text(encoding="utf-8")
        self.assertIn("AppName=AUDREY", script)
        self.assertIn("OutputBaseFilename=AUDREY-SETUP", script)
        self.assertIn("AUDREY.bat", script)
        self.assertIn("diagnose-audrey.bat", script)

    def test_fetch_script_mentions_official_runtime_urls(self):
        script = (ROOT / "installer" / "fetch-runtime-assets.ps1").read_text(encoding="utf-8")
    ...
    def test_build_script_stages_ollama_installer_before_compile(self):
        ...
        self.assertIn("audrey_llm", script)
    ...
    def test_build_guide_mentions_python_embed_wheelhouse_and_assets(self):
        guide = (ROOT / "installer" / "README-BUILD-INSTALLER.md").read_text(encoding="utf-8")
        self.assertIn("python-embed", guide)
        self.assertIn("wheelhouse", guide)
        self.assertIn("audrey.ico", guide)
        self.assertIn("classy.png", guide)

    def test_env_example_mentions_deepseek(self):
        env_example = (ROOT / ".env.example").read_text(encoding="utf-8")
        self.assertIn("AUDREY_DEFAULT_BACKEND=deepseek", env_example)
